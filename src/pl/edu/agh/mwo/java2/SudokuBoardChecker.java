...
public class SudokuBoardChecker {
	...
	public boolean verifyBoardStructure(int sheetIndex) {
		...
		// 3 warunki, ktore moga zmienic zmienna poprawnosc: nie ma wiersza, nie ma komorki, komorka inna niz pusty lub cyfry  
		
		//iteracja po wierszach 
		for (int wiersz=0;wiersz<9;wiersz++) {
			// jesli tylko poprawnosc jest false, to porzuca dalsza prace
			if (poprawnosc==false) break;
	...
	public boolean verifyBoard(int sheetIndex) {
		...
		/* 
		 * ALGORYTM:
		 *  a) unikalne liczby w kazdej kolumnie (0-9)
		 *  b) unikalne liczby w kazdym wierszu (0-9)
		 *  c) unikalne liczby w kazdym kwadracie 3x3, (0-9 kwadratow)
		 *   - algorytm dla kwadratow, kazdy kwadrat identifikuja wspolrzedne (x,y)
		 *   - inkrementacja wiersza (x) od 0 do 3, przy stepie +3 dla wiersza
		 *   - dla kazdego wiersza (x) inkrementacja kolumny (y) od 0 do 3 ze stepem +3 
		 */
		...
		// na koniec porownujemy wielkosc dwoch collection, jak jest rozny, to poprawnoscWierszy false i break
		
		for (ArrayList<Cell> element: listaWierszy) {
			// warunek na cel dalszego sprawdzania
			if (poprawnoscWierszy == false) break;
			...
			if (lista.size()!=zbior.size()) poprawnoscWierszy=false;
		}
		
		// 2. sprawdzenie poprawnosci kolumn
		for (int kolumna=0;kolumna<9;kolumna++) {
			// warunek na cel dalszego sprawdzania
			if (poprawnoscKolumn == false) break;
			// collections do porownania
			ArrayList<Double> listaKolumn=new ArrayList<Double>();
			HashSet<Double> zbiorKolumn=new HashSet<Double>();
			// dla i kolumn od 0 do 9 kolumn pobierz wszystkie wiersze, pobierz z nich element Cell pod indeksem i i zapisz do List i Set
			for (ArrayList<Cell> element: listaWierszy) {
			...
			//porownanie czy sa powtorzenia
			if (listaKolumn.size()!=zbiorKolumn.size()) poprawnoscWierszy=false;
			}
		}
		...
		for (int w=0;w<9;w+=3) {
			// warunek na cel dalszego sprawdzania
			if (poprawnoscKwadratow==false) break;
			for (int k=0;k<9;k+=3) {
				// warunek na cel dalszego sprawdzania
				if (poprawnoscKwadratow==false) break;
				...
				for (int i=0;i<3;i++) {
					for (int j=0;j<3;j++) {
						...
						//String s=String.format("kwadrat: (%d,%d)\t wiersz: %d, kolumna: %d", w, k, w+i, k+j);
